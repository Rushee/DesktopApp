       ...
       // $.noConflict();
        ...
        return {
            ...
            _setupEventDelegation: function () {
                ...
                this.element.click(function (event) {   // Click event on element************************
                    var $target = $(event.target),
                      freeBusyManager;
                    //  air.Introspector.Console.log('Click event on element');
                    //  alert('Click event on element');
                    //  click is disabled
                    ...
                    var $calEvent = $target.hasClass('wc-cal-event') ?
                        $target :
                      $target.parents('.wc-cal-event');
                    //  air.Introspector.Console.log($calEvent);
                    if (!$calEvent.length || !$calEvent.data('calEvent')) {
                        return;
                    }
            ...
            _renderCalendarButtons: function ($calendarContainer) {
                ...
                if (options.buttons) {
                    ...
                    calendarNavHtml += '</div>';
                    calendarNavHtml += '<h1 class=\"wc-title\"></h1>'; // Week start and end Date displayed on top
                    calendarNavHtml += '</div>';
                    ...
                    if (this.options.switchDisplay && $.isPlainObject(this.options.switchDisplay)) {
                        ...
                        $.each(this.options.switchDisplay, function (label, option) {
                            ...
                            if (parseInt(self.options.daysToShow, 10) === parseInt(option, 10)) {
                               ...
                               //air.Introspector.Console.log(_input);
                            }
                            $container
                              .append(_input)
                              .append(_label);
                    ...
                    $calendarContainer.find('.wc-title')            // Week start and end Date displayed on top
                      .height(_height)
          ...
          TODO    if total height for an hour is less than 11px, there is a display problem.
            ...
            _setupEventCreationForWeekDay: function ($weekDay) {
                var self = this;
                var options = this.options; //submit
            ...
            _updateDayColumnHeader: function ($weekDayColumns) {
              ...
              //  air.Introspector.Console.log(currentDay);
                ...
                // now update the calendar title ************* Calender header Week start and end date************** 
                if (this.options.title) {
                    ...
                    //air.Introspector.Console.log(end);
                    //air.Introspector.Console.log(date_format);
            ...
            _renderEvent: function (calEvent, $weekDay) {
                ...
                eventHtml += '<div class=\"wc-time ui-corner-top\"></div>';  //Time of event
                eventHtml += '<div class=\"wc-title\"></div></div>';
                //eventHtml += '<div class=\"wc-title\"></div><img class="taskStatusImg" src=""> </img></div>';
                 
                $weekDay.each(function () {
                    var $calEvent = $(eventHtml);
                    $modifiedEvent = options.eventRender(calEvent, $calEvent);
                    $calEvent = $modifiedEvent ? $modifiedEvent.appendTo($(this)) : $calEvent.appendTo($(this));
                    $calEvent.css({ lineHeight: (options.textSize + 2) + 'px', fontSize: '9px' });
